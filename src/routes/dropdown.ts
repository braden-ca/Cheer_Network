...
// Get all specialties for dropdown (public for registration)
...
// Add new specialty
router.post('/specialties', authenticateToken, async (req, res) => {
  try {
    const { name } = req.body;
...
// Get all schools for dropdown (public for registration)
  ...
  try {
    ...
    if (error) {
      ...
      if (error.code === '42P01') {
        ...
        if (search && typeof search === 'string') {
          filteredSchools = defaultSchools.filter(school => 
...
// Get US states for dropdown (public for registration)
